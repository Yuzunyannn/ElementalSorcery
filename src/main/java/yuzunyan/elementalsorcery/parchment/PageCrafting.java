...
public class PageCrafting extends Page {
	...
	private boolean addESRecipe(ItemStack stack) {
		yuzunyan.elementalsorcery.api.crafting.IRecipe irecipe = null;
		List<yuzunyan.elementalsorcery.api.crafting.IRecipe> lsit = RecipeManagement.instance.getRecipes();
		for (yuzunyan.elementalsorcery.api.crafting.IRecipe ire : lsit) {
			if (ire.getRecipeOutput().isItemEqual(stack)) {
				irecipe = ire;
				break;
			}
