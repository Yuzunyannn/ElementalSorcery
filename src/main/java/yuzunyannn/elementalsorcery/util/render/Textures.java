package yuzunyannn.elementalsorcery.util.render;

import net.minecraft.util.ResourceLocation;
import yuzunyannn.elementalsorcery.ElementalSorcery;

public class Textures {
	static final public TextureBinder DESTROY_STAGE_9 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_9.png");
	static final public TextureBinder DESTROY_STAGE_8 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_8.png");
	static final public TextureBinder DESTROY_STAGE_7 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_7.png");
	static final public TextureBinder DESTROY_STAGE_6 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_6.png");
	static final public TextureBinder DESTROY_STAGE_5 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_5.png");
	static final public TextureBinder DESTROY_STAGE_4 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_4.png");
	static final public TextureBinder DESTROY_STAGE_3 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_3.png");
	static final public TextureBinder DESTROY_STAGE_2 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_2.png");
	static final public TextureBinder DESTROY_STAGE_1 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_1.png");
	static final public TextureBinder DESTROY_STAGE_0 = new TextureBinder("minecraft",
			"textures/blocks/destroy_stage_0.png");

	static final public ResourceLocation KYNAITE_BLOCK = new ResourceLocation(ElementalSorcery.MODID,
			"textures/blocks/kynaite_block.png");
	static final public ResourceLocation KYNAITE_ORE = new ResourceLocation(ElementalSorcery.MODID,
			"textures/blocks/kynaite_ore.png");
	static final public ResourceLocation ASTONE = new ResourceLocation(ElementalSorcery.MODID,
			"textures/blocks/astone.png");
	static final public ResourceLocation ASTONE_FRAGMENTED = new ResourceLocation(ElementalSorcery.MODID,
			"textures/blocks/astone_fragmented.png");

	static final public ResourceLocation STONE = new ResourceLocation("minecraft", "textures/blocks/stone.png");
}
