...
public class VillageESHall extends Village {
	...
	public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn) {
		...
		if (!this.isZombieInfested) {
			...
			this.setBlockState(worldIn, doorState.withProperty(BlockDoor.HALF, BlockDoor.EnumDoorHalf.UPPER), 6, 1
			...
			this.setBlockState(worldIn, doorState.withProperty(BlockDoor.HALF, BlockDoor.EnumDoorHalf.UPPER), 7, 1
					+ 1, 1, structureBoundingBoxIn);
		}
		// 火把
	...
	private EntityArmorStand setArmorStand(int x, int y, int z, World world) {
		...
		AxisAlignedBB box = new AxisAlignedBB(blockpos.getX(), blockpos.getY(), blockpos.getZ(), blockpos.getX()
				+ 1, blockpos.getY() + 1, blockpos.getZ() + 1);
		List<EntityArmorStand> list = world.getEntitiesWithinAABB(EntityArmorStand.class, box);
		if (list != null && list.size() != 0) return list.get(0);

		EntityArmorStand armorStand = new EntityArmorStand(world, blockpos.getX() + 0.5, blockpos.getY()
				+ 0.5, blockpos.getZ() + 0.5);
	...
	private void randomArmor(EntityArmorStand armorStand, Random random, World world) {
		...
		if (rate < 0.01)
			mainHand = new ItemStack(ESInit.ITEMS.SPELLBOOK);
		else if (rate < 0.03)
			mainHand = new ItemStack(ESInit.BLOCKS.ELEMENTAL_CUBE);
		else if (rate < 0.075)
			mainHand = new ItemStack(ESInit.ITEMS.ELEMENT_CRYSTAL);
		else if (rate < 0.125)
			mainHand = new ItemStack(ESInit.BLOCKS.MD_INFUSION);
		else if (rate < 0.3)
			mainHand = new ItemStack(ESInit.ITEMS.PARCHMENT, 8);
		else if (rate < 0.5)
			mainHand = new ItemStack(ESInit.BLOCKS.KYANITE_BLOCK, 2);
		else
			mainHand = new ItemStack(ESInit.ITEMS.KYANITE_SWORD);
	...
	public static class VillageCreationHandler implements VillagerRegistry.IVillageCreationHandler {
		...
		public PieceWeight getVillagePieceWeight(Random random, int size) {
			return new StructureVillagePieces.PieceWeight(VillageESHall.class, 18, MathHelper.getInt(random, 0 + size, 0
					+ size));
		}
		...
		public Village buildComponent(PieceWeight villagePiece, Start startPiece, List<StructureComponent> pieces,
			...
			StructureBoundingBox structureboundingbox = StructureBoundingBox.getComponentToAddBoundingBox(p1, p2, p3, 0, 0, 0, 14, 6, 12, facing);
			return canVillageGoDeeper(structureboundingbox)
					&& StructureComponent.findIntersecting(pieces, structureboundingbox) == null ? new VillageESHall(startPiece, p5, random, structureboundingbox, facing) : null;
		}
