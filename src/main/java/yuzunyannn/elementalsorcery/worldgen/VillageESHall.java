...
public class VillageESHall extends Village {
	...
	private void randomArmor(EntityArmorStand armorStand, Random random, World world) {
		...
		armorStand.setItemStackToSlot(EntityEquipmentSlot.OFFHAND, new ItemStack(ESInitInstance.BLOCKS.ESTONE));
		float rate = random.nextFloat();
		ItemStack mainHand = ItemStack.EMPTY;
		if (rate < 0.01)
			mainHand = new ItemStack(ESInitInstance.ITEMS.SPELLBOOK);
		else if (rate < 0.03)
			mainHand = new ItemStack(ESInitInstance.BLOCKS.ELEMENTAL_CUBE);
		else if (rate < 0.075)
	...
	protected VillagerProfession chooseForgeProfession(int count, VillagerProfession prof) {
		return ESInitInstance.VILLAGE.ES_VILLEGER;
	}
