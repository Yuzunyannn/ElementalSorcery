...
import yuzunyannn.elementalsorcery.container.ContainerItemStructureCraftCC;
...
public class GuiItemStructureCraftCC extends GuiContainer {
	...
	protected final ContainerItemStructureCraftCC container;
	...
	class SlotView {
		...
		public ItemStack getItemStack() {
			...
			if (spIndex == ContainerItemStructureCraftCC.SP_OUTPUT) {
				stack = container.tileEntity.getOutput();
			} else if (spIndex == ContainerItemStructureCraftCC.SP_TYPE_STACK) {
	...
	public GuiItemStructureCraftCC(ContainerItemStructureCraftCC inventorySlotsIn) {
	...
	protected void drawSlot(boolean hasSelect) {
		RenderHelper.drawTexturedRectInCenter(0, 0, 18, 18, 230, 0, 18, 18, 256, 256);
		if (hasSelect) RenderHelper.drawTexturedRectInCenter(0, 0, 18, 18, 230, 18, 18, 18, 256, 256);
	}
	...
	public void updateScreen() {
		...
		if (!container.lastSlotMapName.equals(container.tileEntity.getTypeName())) {
			...
			if (!slotViewList.isEmpty()) {
				SlotView view = new SlotView(-1, ContainerItemStructureCraftCC.SP_OUTPUT);
				view.prevX = view.x = cX + xLen / 2 - 9;
				view.prevY = view.y = cY + yLen / 2 - 9;
				slotViewList.add(view);
			}
			if (hasTypeStack) {
				SlotView view = new SlotView(-1, ContainerItemStructureCraftCC.SP_TYPE_STACK);
				view.prevX = view.x = cX + xLen / 2 - 9;
				view.prevY = view.y = cY + yLen / 2 - 9;
				slotViewList.add(view);
			}
		...
		for (SlotView view : slotViewList) {
			if (view.spIndex == ContainerItemStructureCraftCC.SP_OUTPUT) {
				view.update(cX + xLen + 18, cY + yLen / 2 - 9);
				continue;
			} else if (view.spIndex == ContainerItemStructureCraftCC.SP_TYPE_STACK) {
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		for (SlotView view : slotViewList) {
			if (view.spIndex == ContainerItemStructureCraftCC.SP_OUTPUT) continue;
			if (GuiNormal.isMouseIn(mouseX, mouseY, view.x, view.y, 18, 18)) {
				if (view.spIndex == ContainerItemStructureCraftCC.SP_TYPE_STACK) {
					this.sendOP(view.slotIndex, ContainerItemStructureCraftCC.OP_TSTACK);
					return;
				}
				int x = (view.slotIndex >> 16) & 0xffff;
				int y = view.slotIndex & 0xffff;
	...
	protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
		for (SlotView view : slotViewList) {
			if (view.spIndex == ContainerItemStructureCraftCC.SP_OUTPUT) continue;
			else if (view.spIndex == ContainerItemStructureCraftCC.SP_TYPE_STACK) continue;
			if (GuiNormal.isMouseIn(mouseX, mouseY, view.x, view.y, 18, 18)) {
				int x = (view.slotIndex >> 16) & 0xffff;
				int y = view.slotIndex & 0xffff;
	...
	protected void mouseClickedSlot(int x, int y, int mouseButton) {
		...
		if (lastClickTick > 0 && lastDealIndex == index && mouseButton == 0) {
			...
			for (Entry<Integer, ItemStack> entry : slotMap.entrySet()) {
				if (ItemHelper.areItemsEqual(entry.getValue(), lastRemove)) {
					this.sendOP(entry.getKey(), ContainerItemStructureCraftCC.OP_REMOVE_SAME);
					return;
				}
			}
			return;
		...
		if (playerHold.isEmpty()) {
			...
			if (mouseButton == 0) {
				...
				this.sendOP(x, y, ContainerItemStructureCraftCC.OP_REMOVE);
			} else if (mouseButton == 1) this.sendOP(x, y, ContainerItemStructureCraftCC.OP_REMOVE_HALF);
		} else {
			if (slotItem.isEmpty()) {
				if (mouseButton == 0) this.sendOP(x, y, ContainerItemStructureCraftCC.OP_ADD);
				else if (mouseButton == 1) this.sendOP(x, y, ContainerItemStructureCraftCC.OP_ADD_HALF);
	...
	protected void mouseClickedMoveSlot(int x, int y, int mouseButton) {
		...
		if (playerHold.isEmpty()) {
			if (slotItem.isEmpty()) return;
			if (mouseButton == 0) this.sendOP(x, y, ContainerItemStructureCraftCC.OP_REMOVE);
			else if (mouseButton == 1) this.sendOP(x, y, ContainerItemStructureCraftCC.OP_REMOVE_HALF);
		} else {
			if (slotItem.isEmpty()) {
				if (mouseButton == 0) this.sendOP(x, y, ContainerItemStructureCraftCC.OP_ADD);
