...
public class RecipePageBuilding extends net.minecraftforge.registries.IForgeRegistryEntry.Impl<IRecipe>
	...
	public boolean matches(InventoryCrafting inv, World worldIn) {
		...
		for (int i = 0; i < inv.getSizeInventory(); i++) {
			...
			} else if (stack.getItem() == ESInitInstance.ITEMS.ARCHITECTURE_CRYSTAL) {
	...
	public ItemStack getCraftingResult(InventoryCrafting inv) {
		...
		for (int i = 0; i < inv.getSizeInventory(); i++) {
			ItemStack stack = inv.getStackInSlot(i);
			if (stack.isEmpty()) continue;
			if (stack.getItem() == ESInitInstance.ITEMS.PARCHMENT) parchment = stack;
			else if (stack.getItem() == ESInitInstance.ITEMS.ARCHITECTURE_CRYSTAL) archCrystal = stack;
		}
