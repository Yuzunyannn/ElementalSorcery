...
public class RecipeGrimoire extends Recipe {
	...
	public RecipeGrimoire() {
		super(new ItemStack(ESObjects.ITEMS.GRIMOIRE));
		List<String> ss = Arrays.asList(new String[] { "LPL", "SPL", "LPL", " GG ", "GLLG", "GLLG", " GG " });
		Map<String, ItemStack[]> map = new HashMap<String, ItemStack[]>();
		map.put("S", ItemHelper.toArray(ESObjects.ITEMS.ELEMENT_STONE));
		map.put("L", ItemHelper.toArray(ESObjects.ITEMS.LIFE_LEATHER, 1, 1));
		map.put("P", ItemHelper.toArray(ESObjects.ITEMS.MAGIC_PAPER, 1, 3));
		map.put("G", ItemHelper.toArray(Items.GOLD_INGOT));
		List<ElementStack> needElement = new ArrayList<ElementStack>();
		needElement.add(new ElementStack(ESObjects.ELEMENTS.KNOWLEDGE, 300, 200));
