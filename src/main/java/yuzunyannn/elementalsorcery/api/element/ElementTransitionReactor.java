...
public class ElementTransitionReactor {
	...
	public void transitTo(Element element) {
		...
		step = nEt.getLevel();
		if (oEt == null) return;
		fragment = ElementTransition.transitionFrom(element, fragment, oEt.getLevel());
		fragment = ElementTransition.transitionTo(element, fragment, nEt.getLevel());
	}
	...
	public ElementStack extract(double cost, int maxSize, int power, boolean simulate) {
		...
		double countDouble = ElementTransition.fromFragmentByPower(this.rElement, costFragment, power);
		...
		this.fragment = this.fragment - costFragment
				+ ElementTransition.toFragment(this.rElement, countDouble - count, power);
		return eStack;
	}
